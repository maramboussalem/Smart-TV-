...
QDate employes::getDate_naissance(){return date_naissance;}
...
void employes::setDate_Naissance(QDate d) { date_naissance = d; }
...
void employes::Recherche(int id_E)
{
    QSqlQuery query;
    ...
    if (query.exec() && query.next()) {
        ...
        QString telephonne = query.value(5).toString();
...
void employes::Email(const QString &email)
{
...
QSqlQueryModel * employes::rechercher_M(QString adresse)
{
    QSqlQueryModel *model = new QSqlQueryModel();
    if(adresse.isEmpty()) {
        model->setQuery("SELECT * FROM employes");
